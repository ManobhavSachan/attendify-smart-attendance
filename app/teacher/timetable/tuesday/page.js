...
export default function Timetable() {
  ...
  useEffect(() => {
    const savedTimetable = localStorage.getItem('tttuesday');
    if (savedTimetable) {
  ...
  const addItem = () => {
    if (newDay.trim() !== '' && newTime.trim() !== '') {
      ...
      localStorage.setItem('tttuesday', JSON.stringify([...timetable, newItem]));
      setNewDay('');
      setNewTime('');
      setNewcr('');
    }
  ...
  const deleteItem = (index) => {
    ...
    localStorage.setItem('tttuesday', JSON.stringify(updatedTimetable));
  };
