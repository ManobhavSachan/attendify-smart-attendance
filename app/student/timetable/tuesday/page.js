...
export default function Timetable() {
  ...
  useEffect(() => {
    const savedTimetable = localStorage.getItem('timetabletuesday');
    if (savedTimetable) {
  ...
  const addItem = () => {
    if (newDay.trim() !== '' && newTime.trim() !== '') {
      ...
      localStorage.setItem('timetabletuesday', JSON.stringify([...timetable, newItem]));
      setNewDay('');
      setNewTime('');
      setNewcr('');
    }
  ...
  const deleteItem = (index) => {
    ...
    localStorage.setItem('timetabletuesday', JSON.stringify(updatedTimetable));
  };
  ...
  return (
    <div className={styles.homecontainer}>
      ...
      <h1 className={styles.hometext}> Tuesday</h1>
