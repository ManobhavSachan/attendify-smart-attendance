...
export default function Timetable() {
  ...
  useEffect(() => {
    const savedTimetable = localStorage.getItem('timetablesaturday');
    if (savedTimetable) {
  ...
  const addItem = () => {
    if (newDay.trim() !== '' && newTime.trim() !== '') {
      ...
      localStorage.setItem('timetablesaturday', JSON.stringify([...timetable, newItem]));
      setNewDay('');
      setNewTime('');
      setNewcr('');
    }
  ...
  const deleteItem = (index) => {
    ...
    localStorage.setItem('timetablesaturday', JSON.stringify(updatedTimetable));
  };
