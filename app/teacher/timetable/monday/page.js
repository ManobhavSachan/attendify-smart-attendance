...
export default function Timetable() {
  ...
  useEffect(() => {
    const savedTimetable = localStorage.getItem('ttmonday');
    if (savedTimetable) {
  ...
  const addItem = () => {
    if (newDay.trim() !== '' && newTime.trim() !== '') {
      ...
      localStorage.setItem('ttmonday', JSON.stringify([...timetable, newItem]));
      setNewDay('');
      setNewTime('');
      setNewcr('');
    }
  ...
  const deleteItem = (index) => {
    ...
    localStorage.setItem('ttmonday', JSON.stringify(updatedTimetable));
  };
