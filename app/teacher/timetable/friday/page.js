...
export default function Timetable() {
  ...
  useEffect(() => {
    const savedTimetable = localStorage.getItem('ttfriday');
    if (savedTimetable) {
  ...
  const addItem = () => {
    if (newDay.trim() !== '' && newTime.trim() !== '') {
      ...
      localStorage.setItem('ttfriday', JSON.stringify([...timetable, newItem]));
      setNewDay('');
      setNewTime('');
      setNewcr('');
    }
  ...
  const deleteItem = (index) => {
    ...
    localStorage.setItem('ttfriday', JSON.stringify(updatedTimetable));
  };
